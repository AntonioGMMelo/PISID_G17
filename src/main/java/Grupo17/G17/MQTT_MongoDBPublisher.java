...
public class MQTT_MongoDBPublisher implements MqttCallbackExtended{
	...
	public MQTT_MongoDBPublisher(String serverURI) {
	 ...
	 public synchronized void publicar(String topic, byte[] payload, int qos, boolean retained) {
		 ...
		 try {
			 MqttClient mqttClient = new MqttClient("127.0.0.1:27017", MqttClient.generateClientId());
			 mqttClient.connect();
			 MqttMessage message = new MqttMessage();
			 message.setPayload(message.getPayload());
			 client.publish("test", message); //(topic, message)
			 System.out.println("Tópico publicado!");
			 System.out.println("\tMessage: " + message + " to test.");
			 client.disconnect();
